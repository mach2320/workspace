...
async function insertCartProduct(param){
    let connection = await oracledb.getConnection(ORACLE_CONFIG);
    var sql2 = "INSERT INTO CARTPRODUCT(cartproduct_id, product_id, cartproduct_count, cart_id) VALUES ( (SELECT NVL(MAX(CARTPRODUCT_ID),0 + 1 FROM CARTPRODUCT),:productId, :count,(SELECT CART_ID FROM CART WHERE USER_ID =:userId)) "
    let options = {
        outFormat: oracledb.OUT_FORMAT_OBJECT
    };
